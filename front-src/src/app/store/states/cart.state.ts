...
import {AddProductToCart, DeleteProduct, UpdateCost} from "../actions/cart.actions";
...
export class CartState {
  ...
  @Selector()
  static cartCost(state: CartStateModel): number {
    return state.items.map(v => v.cost * v.count).reduce((sum, current) => sum + current, 0)
  }

  @Action(DeleteProduct)
  deleteProductFromCart(ctx: StateContext<CartStateModel>, {payload}: DeleteProduct) {
    ctx.setState(
      patch({
        items: removeItem<CartItem>(i => i?.productId == payload)
      })
    );
  }
